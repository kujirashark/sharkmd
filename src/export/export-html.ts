...
/**
 * Word format. The simplest path that round-trips reasonably: write the
 * HTML to a .doc file. Word opens it as HTML and lets the user save as .docx.
 * The MIME type is set so browsers / Word recognise the intent.
 */
export function exportToWord(doc: JSONContent, opts: ExportOptions): string {
  // For .doc we use the same HTML but with an explicit MSO namespace and
  // XML declaration so Word treats it as a true Word document.
  // Synchronous rendering — skip KaTeX/Shiki heavy async, use text fallback.
  const fallbackRender = (node: JSONContent): string => {
    if (node.type === 'text') {
      let html = escapeHtml(node.text ?? '');
      const marks = node.marks ?? [];
      for (const m of marks) {
        if (m.type === 'bold') html = `<b>${html}</b>`;
        else if (m.type === 'italic') html = `<i>${html}</i>`;
        else if (m.type === 'strike') html = `<s>${html}</s>`;
        else if (m.type === 'code') html = `<code>${html}</code>`;
        else if (m.type === 'link') {
          const href = (m.attrs as any)?.href ?? '#';
  ...
  const body = (doc.content ?? []).map(fallbackRender).join('\n');
  const date = new Date().toISOString().slice(0, 10);
...
code { font-family: "Consolas", monospace; background: #f0f0f0; padding: 1px 4px; }
pre { background: #f0f0f0; padding: 8px; }
blockquote { border-left: 3px solid #ccc; padding-left: 1em; color: #555; }
</style>
</head>
<body>
<p style="color:#666;font-size:12px">${escapeHtml(opts.author ?? 'sharkmd')} · ${date}</p>
${body}
</body>
</html>`;
}
